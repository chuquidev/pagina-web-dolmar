...
import { ShoppingBag, ShoppingCart, Menu, X, Search, Home, LayoutGrid, Users, Wrench, ChevronRight } from '@lucide/vue'
...
const navItems = [
    ...
    { to: '/reservar-mantenimiento', label: 'Mantenimiento', icon: Wrench, exact: false },
]
...
<template>
    <header
        ...
        <div class="mx-auto flex max-w-[1400px] items-center gap-3 px-4 py-3 lg:gap-4">
            <RouterLink to="/" class="flex shrink-0 items-center gap-2.5 font-display font-bold text-brand-primary">
                <img v-if="settingsStore.settings?.logo" :src="settingsStore.settings.logo"
                    :alt="settingsStore.settings.store_name" class="h-12 w-12 shrink-0 rounded-lg object-contain" />
                <ShoppingBag v-else class="h-10 w-10 shrink-0" />
                <span class="hidden truncate text-lg tracking-tight sm:inline">{{ settingsStore.settings?.store_name
                    ?? 'Cargando...' }}</span>
            </RouterLink>
            ...
            <nav class="hidden shrink-0 items-center gap-1 lg:ml-3 lg:flex">
                <RouterLink v-for="item in desktopNavItems" :key="item.to" :to="item.to" custom
                    ...
                    <a :href="href"
                        ...
                        <span
                            ...
                            :class="(item.exact ? isExactActive : isActive) ? 'scale-x-100' : 'scale-x-0'" />
                    </a>
                </RouterLink>
            </nav>
        ...
        <Teleport to="body">
            ...
            <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
                ...
                <div v-if="mobileMenuOpen" class="fixed inset-0 z-40 bg-gray-900/60 backdrop-blur-sm lg:hidden"
                ...
                <nav v-if="mobileMenuOpen"
                    ...
                    <div
                        ...
                        <RouterLink to="/"
                            ...
                            <span class="truncate text-lg tracking-tight">{{ settingsStore.settings?.store_name ??
                                'Cargando...' }}</span>
                        </RouterLink>
                        <button
                            class="rounded-lg p-2 text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800"
                            aria-label="Cerrar menú" @click="mobileMenuOpen = false">
                            <X class="h-6 w-6" />
                        </button>
                    </div>
                    ...
                    <div class="flex flex-1 flex-col overflow-y-auto px-3 py-4">
                        ...
                        <div class="flex flex-col gap-1.5">
                            <RouterLink v-for="(item, i) in mobileNavItems" :key="item.to" :to="item.to" custom
                                v-slot="{ href, navigate, isExactActive, isActive }">
                                <a :href="href"
                                    ...
                                    :style="{ animationDelay: `${i * 60}ms` }" :class="(item.exact ? isExactActive : isActive)
                                        ? 'text-brand-primary'
                                        : 'text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-800'"
                                    @click="navigate($event); mobileMenuOpen = false">
                                    <span
                                        ...
                                        :class="(item.exact ? isExactActive : isActive) ? 'opacity-100' : 'opacity-0'" />
                                    <span
                    ...
                    <div class="shrink-0 space-y-3 border-t border-gray-100 px-4 py-4 dark:border-gray-800">
                        ...
                        <p v-if="settingsStore.settings?.store_name"
                            class="text-center text-xs text-gray-400 dark:text-gray-500">
                            {{ settingsStore.settings.store_name }}<template v-if="settingsStore.settings?.address">
