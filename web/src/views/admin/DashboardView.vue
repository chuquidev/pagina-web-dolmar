...
import { Package, FolderTree, Tags, Star, ImageOff, Percent, Power, Clock } from '@lucide/vue'
    ...
    <div>
        ...
        <template v-else-if="stats">
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <StatCard label="Marcas" :value="stats.total_brands" :icon="Tags" />
            ...
            <!-- Fila secundaria: estado del catálogo -->
            <div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <StatCard label="Activos / Inactivos" :value="`${stats.active_products} / ${stats.inactive_products}`"
                    :icon="Power" icon-bg="bg-green-50 dark:bg-green-950/40"
            ...
            <p v-if="stats.products_without_images > 0"
                class="mt-3 flex items-center gap-2 rounded-lg bg-amber-50 px-4 py-2.5 text-sm text-amber-700 dark:bg-amber-950/40 dark:text-amber-400">
                <ImageOff class="h-4 w-4 shrink-0" />
                Tienes {{ stats.products_without_images }} producto{{ stats.products_without_images === 1 ? '' : 's' }}
                sin fotos — los clientes ven un ícono genérico en vez de la imagen real.
            ...
            <div class="mt-6 rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
                ...
                <ul v-if="stats.recent_products.length" class="mt-4 divide-y divide-gray-100 dark:divide-gray-800">
                    <li v-for="product in stats.recent_products" :key="product.id" class="flex items-center gap-3 py-3">
                        <div class="h-11 w-11 shrink-0 overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="truncate font-medium text-gray-900 dark:text-gray-100">{{ product.name }}</p>
                            <p class="text-xs text-gray-400 dark:text-gray-500">{{ product.category }} · S/ {{
                                product.price }}</p>
                        </div>
