...
// Título y descripción propios de esta página: sin esto, /catalogo se quedaba
// con el <title> y <meta description> por defecto de index.html — los mismos
// que la home — y Google terminaba mostrando ambas URLs como si fueran
// prácticamente la misma página.
const activeCategoryName = computed(() => {
  const slug = route.query.category as string | undefined;
  if (!slug) return null;
  return catalogStore.categories.find((c) => c.slug === slug)?.name ?? null;
});

useHead(() => {
  const storeName = settingsStore.settings?.store_name ?? "nuestra tienda";
  const category = activeCategoryName.value;
  const title = category
    ? `${category} — Catálogo — ${storeName}`
    : `Catálogo — ${storeName}`;
  const description = category
    ? `${category} en ${storeName}: revisa precios, stock y disponibilidad, y compra directo por WhatsApp.`
    : `Catálogo completo de ${storeName}: bicicletas, repuestos y accesorios. Consulta precios y disponibilidad, y compra directo por WhatsApp.`;
  ...
  return {
    ...
    meta: [
      ...
      // No queremos que /catalogo aparezca como resultado en Google (solo la
      // home). "follow" deja que igual rastree los links a cada producto,
      // así las fichas individuales sí se pueden indexar.
...
<template>
  <div class="mx-auto max-w-[1400px] px-4 py-6 sm:py-8">
    ...
    <h1
      class="font-display text-xl font-bold text-gray-900 dark:text-gray-100 sm:text-2xl"
    >
      Catálogo
    </h1>
    ...
    <div class="relative mt-5 sm:mt-6">
      ...
      >
        <select
          ...
          @change="
            updateFilter({
              category: ($event.target as HTMLSelectElement).value || undefined,
            })
          "
        >
          <option value="">Todas las categorías</option>
          <option
            v-for="c in catalogStore.categories"
            :key="c.id"
            :value="c.slug"
          >
            {{ c.name }}
        ...
        <select
          ...
          @change="
            updateFilter({
              brand: ($event.target as HTMLSelectElement).value || undefined,
            })
          "
        >
          <option value="">Todas las marcas</option>
          <option v-for="b in catalogStore.brands" :key="b.id" :value="b.slug">
            {{ b.name }}
          </option>
        </select>
        ...
        <select
          ...
          class="shrink-0 rounded-full border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 sm:px-4"
          @change="
            updateFilter({ sort: ($event.target as HTMLSelectElement).value })
          "
        >
          <option
    ...
    <div
      v-if="loading"
      class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4 xl:grid-cols-5"
    >
      <SkeletonCard v-for="n in 10" :key="n" />
    </div>
    <div
      v-else-if="!products.length"
      class="py-24 text-center text-gray-400 dark:text-gray-500"
    >
